...
proc genMarker(g: LLGen, typ: PType, n: PNode, v, op: llvm.ValueRef, start: var int) =
  ...
  of nkRecCase:
    let
      kind = n[0].sym
      gep = g.b.buildGEP(v, [g.gep0, g.constGEPIdx(start)], g.nn("mk.kind", kind))
...
proc llProcParamType(g: LLGen, t: PType): llvm.TypeRef =
  ...
  of tySet:
    let size = g.config.getSize(typ).cuint
    if size <= 8: g.llType(t)
    else: g.llType(t).pointerType()
  of tyProc:
    if typ.callConv == ccClosure: g.llType(t).pointerType()
    else: g.llType(t)
  of tyGenericBody, tyGenericInst, tyGenericInvocation, tyGenericParam,
      tyDistinct, tyOrdinal, tyTypeDesc, tyAlias, tySink, tyUserTypeClass,
      tyUserTypeClassInst, tyInferred, tyStatic:
    g.llProcParamType(t.lastSon)
  else:
    g.llType(t)
...
proc llProcType(g: LLGen, typ: PType, closure: bool): llvm.TypeRef =
  ...
  for param in typ.procParams():
    ...
    if g.llPassAsPtr(param.sym.typ):
      incl(param.sym.loc.flags, lfIndirect)
      param.sym.loc.storage = OnUnknown

    let at = g.llProcParamType(param.sym.typ)
    argTypes.add(at)
...
proc callCompilerProc(
  ...
  for param in sym.typ.procParams():
    ...
    # TODO unsigned
    let a = g.preCast(false, v, param.sym.typ, g.llProcParamType(param.sym.typ))
    args[i] = a
...
proc genResetN(g: LLGen, accessor: llvm.ValueRef, n: PNode, typ: PType, start: var int) =
  ...
  of nkRecCase:
    let
      kind = n[0].sym
      gep = g.b.buildGEP(accessor, [g.gep0, g.constGepIdx(start)], g.nn("reset.kind", kind))
      vk = g.b.buildLoad(gep, g.nn("reset.kind.load", kind))
      caseend = g.b.appendBasicBlockInContext(g.lc, g.nn("reset.kind.end", kind))
    ...
    for i in 1..<n.len:
      let
        branch = n[i]
        ctrue = g.b.appendBasicBlockInContext(g.lc, g.nn("reset.kind.true", branch))
      ...
      if branch.kind == nkOfBranch:
        ...
        for j in 0 .. length-2:
          ...
          if branch[j].kind == nkRange:
            ...
            let scmp = g.b.buildICmp(
              llvm.IntSGE, vk, llvm.constInt(vk.typeOfX(), s.culonglong, llvm.True),
                g.nn("reset.kind.rng.s", branch))
            let ecmp = g.b.buildICmp(
              llvm.IntSLE, vk, llvm.constInt(vk.typeOfX(), e.culonglong, llvm.True),
                g.nn("reset.kind.rng.e", branch))

            cmp = g.b.buildAnd(scmp, ecmp, g.nn("reset.kind.rng.cmp", branch))

          else:
            let k = branch[j].intVal
            cmp = g.b.buildICmp(
              llvm.IntEQ, vk, llvm.constInt(vk.typeOfX(), k.culonglong, llvm.True),
              g.nn("reset.kind.cmp", branch))

          let cfalse = g.b.appendBasicBlockInContext(
            g.lc, g.nn("reset.kind.false", branch))
          discard g.b.buildCondBr(cmp, ctrue, cfalse)
          g.b.positionBuilderAtEnd(cfalse)
      else:
        hasElse = true
...
proc genResetT(g: LLGen, typ: PType, v: llvm.ValueRef) =
  ...
  of tyProc:
    if typ.callConv == ccClosure:
      let p = g.b.buildGEP(v, [g.gep0, g.gep1])
      discard g.callCompilerProc(
        "unsureAsgnRef", [p, constNull(p.typeOfX().getElementType())])
    else:
      g.buildStoreNull(v)
...
proc genCallArgs(g: LLGen, n: PNode, fxt: llvm.TypeRef, ftyp: PType, extra: int): seq[llvm.ValueRef] =
  ...
  for i in 1..<n.len:
    ...
    if skipTypes(param.sym.typ, abstractVar).kind in {tyOpenArray, tyVarargs}:
      ...
      if v.typeOfX() != g.llProcParamType(param.sym.typ):
        v = g.b.buildBitCast(v, g.llProcParamType(param.sym.typ), g.nn("call.open", v))

      args.add(v)
      args.add(len)
    else:
      v = g.genNode(p, not g.llPassAsPtr(param.sym.typ)).v
...
proc genCall(g: LLGen, n: PNode, load: bool): LLValue =
  ...
  let
    nf = n[namePos]
    typ = nf.typ.skipTypes(abstractInst)
    ...
    retArgs =
      if g.isInvalidReturnType(retArgType):
        # TODO this should become RVO when applicable
...
proc genAssignment(
  ...
  of tyTuple, tyArray:
    ...
    if srcx.v != nil:
      if ty.supportsCopyMem():
        g.callMemcpy(dest.v, srcx.v, g.constStoreSize(destet))
      else:
  ...
  of tyOpenArray, tyVarargs:
    ...
    if ty.supportsCopyMem():
      g.callMemcpy(dest.v, p, g.constStoreSize(destet))
    else:
      discard g.callCompilerProc(
...
proc genMagicNewFinalize(g: LLGen, n: PNode) =
  ...
  let
    src = LLValue(
      v: g.callCompilerProc("newObj", [ti, sizeOfExpr]),
      storage: OnHeap)
