...
proc moduleCachePrune*(
  path: cstring, pruneInterval: Time, pruneAfter: Time
) {.importc: "clang_ModuleCache_prune", dynlib: CLangLib.}
...
type CXModuleCachePruneCallback* = proc(path: cstring, userData: pointer)
...
proc moduleCachePruneWithCallback*(
  path: cstring,
  pruneInterval: Time,
  pruneAfter: Time,
  callback: CXModuleCachePruneCallback,
  userData: pointer,
) {.importc: "clang_ModuleCache_pruneWithCallback", dynlib: CLangLib.}
