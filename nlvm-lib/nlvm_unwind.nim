...
type
  ...
  UnwindExceptionCleanupFn* =
    proc(unwindCode: UnwindReasonCode, exception: ptr UnwindException) {.cdecl.}
  UnwindStopFn* = proc(
    version: cint,
...
proc raiseException*(
  exception: ptr UnwindException
): UnwindReasonCode {.importc: "_Unwind_RaiseException".}

proc resume*(exception: ptr UnwindException) {.importc: "_Unwind_Resume".}
proc deleteException*(
  exception: ptr UnwindException
) {.importc: "_Unwind_DeleteException".}

func getGR*(ctx: UnwindContext, reg_index: cint): uint {.importc: "_Unwind_GetGR".}
proc setGR*(
  ctx: UnwindContext, reg_index: cint, value: uint
) {.importc: "_Unwind_SetGR".}
...
func getLanguageSpecificData*(
  ctx: UnwindContext
): uint {.importc: "_Unwind_GetLanguageSpecificData".}
proc forcedUnwind*(
  exception: ptr UnwindException, stop: UnwindStopFn, parameter: pointer
) {.importc: "_Unwind_ForcedUnwind".}

proc resumeOrRethrow*(
  exception: ptr UnwindException
): UnwindReasonCode {.importc: "_Unwind_Resume_Or_Rethrow".}

type UnwindTraceFn* = proc(ctx: UnwindContext, parameter: pointer) {.cdecl.}
proc backtrace*(
  traceFn: UnwindTraceFn, parameter: pointer
) {.importc: "_Unwind_Backtrace".}

func getCFA*(ctx: UnwindContext): uint {.importc: "_Unwind_GetCFA".}

proc getIPInfo*(
  ctx: UnwindContext, ipBefore: ptr cint
): uint {.importc: "_Unwind_GetIPInfo".}

type DwarfEhBases* = object
  tbase*: uint
  dbase*: uint
  fnc*: uint

proc findFDE*(
  pc: pointer, bases: ptr DwarfEhBases
): pointer {.importc: "_Unwind_Find_FDE".}

proc findEnclosingFunction*(
  pc: pointer
): pointer {.importc: "_Unwind_FindEnclosingFunction".}
